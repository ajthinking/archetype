...
class ShallowNodeFinder extends NodeFinder {
    ...
    public function findInstanceOf($node, string $class) : array {
        ...
        return $properties->map(function($property) use ($node, $class) {
            ...
            if(is_object($node->$property) && $node->$property instanceof $class) return $node->$property;
            // if property holds array of objects
            if(is_array($node->$property)) return collect($node->$property)->filter(function($item) use($class) {
                if(is_object($item) && $item instanceof $class) return $item;                
            })->toArray();
